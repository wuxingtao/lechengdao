...
(function ($) {      
	...
	function resetIndex(){
			  ...
			  $("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length-1);
               if(datetime){
                ...
                if(Ycallback===undefined){
                     if(docType){that.val(datestr.substring(datestr.indexOf("-")+1,datestr.length));}else{that.html(datestr);}
                }else{
                     Ycallback(datestr);
                }
        ...
        function createSECOND_UL(){
            var str="<li>&nbsp;</li>";
            str+="<li style='display:none'>上午</li><li style='display:none'>下午</li>"
            return str+"<li>&nbsp;</li>";;
        }
    }
})(jQuery);  
