...
Zzx.prototype ={
	init:function(){
		...
		this.stack = 0;
		//图片路径
		this.imgPath = '../script/source/img/';		
		//定时器
		this.timer = null;			
		//测试JSON数据（可以替换为AJAX请求返回值）
		this.testJson = {
						list:[
								{src:"media-img1_03.png",title:"《陪你度过漫长岁月》",song:"陪你度过漫长岁月",time:"2015年10月6日"},
								{src:"ywaq.jpg",title:"因为爱情-王菲",song:"因为爱情",time:"2015年10月6日"},
								{src:"hnh.jpg",title:"情歌-梁静茹",song:"情歌",time:"2015年10月6日"},
								{src:"ywaq.jpg",title:"因为爱情-王菲",song:"因为爱情",time:"2015年10月6日"},
	...
	loadDate:function(oList){			
		switch(this.type){  
			//根据不同的模块 定制不同的数据展示形式
			case 1:this.ulTarget.append('<li class="artist-song" onclick="myControl.selectList(this,'+this.stack+')">'
										+'<div class="theme-img"><img src="'+this.imgPath+oList[this.stack].src+'"/></div>'
										...
										+'<div class="privatCont"><div class="song-cont"><h3>'+oList[this.stack].title+'</h3><p>'+oList[this.stack].time+'</p></div><div class="btnplay frmPlay"><span><i class="qi-iconfont">&#xe617;</i></span></div></div>'
...
var Control = function(o){
	...
	this.path            = '../script/source/media/';  //歌曲路径（相对于html）
	this.imgPath         = '../script/source/img/';   //图片路径（相对于html）
	this.init();
}
...
var myControl = new Control({
			 audio : document.getElementById("myaudio"), //播放器
